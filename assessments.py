...
links.to_csv('./output/assessment_organization.csv', index=False, encoding='UTF-8')
...
assessments.to_csv('./output/assessments.csv', index=False, encoding='UTF-8')

# Reporting on missing matches

missing = assessments[~ assessments['id'].isin(links['assessment_id'])]
missing = missing[~ missing['is_draft']]
missing.to_csv('./output/missing.csv', index=False, sep=';', encoding='UTF-8')
